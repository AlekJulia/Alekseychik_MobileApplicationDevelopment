...
typedef struct
{
    uint8_t ROM_code[8];    										// 64 bit ROM code
		...
		// поля для поиска датчика
		uint8_t is_present;          // 1 = датчик есть, 0 = пропал
    uint8_t missing_counter;     // счетчик: сколько раз подряд не ответил
		uint8_t alert_was_sent;
} Sensor;
//--------------------------------------------------
#endif /* DS18B20_H_ */